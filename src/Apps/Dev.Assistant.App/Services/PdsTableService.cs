using Dev.Assistant.App.UtilitiesOps;
...
public class PdsTableService
{
    private PdsTableResult _serviceInfo;
    private readonly LogArgs _log; // to access Log in Home Form

    public PdsTableService(LogArgs log)
    {
        _log = log;
    }

    public PdsTableResult ListTablesService(NicServiceFilePath file, ListTablesServiceOptions options = null)
    {
        //_log.LogInfo($"Start with {path}");
        ...
        _serviceInfo = new PdsTableResult
        {
            Name = null,
    ...
    private bool ListTablesForSelectedMicro(string microName, ListTablesServiceOptions options)
    {
        ...
        {
            // TODO: .
            return File(new() { Path = microName, IsRemotePath = options.IsRemote }, options);
        }
        else
        {
            string path;
            ...
            return File(new() { Path = path, }, options);
        }

    }

    private bool File(NicServiceFilePath file, ListTablesServiceOptions options)
    {
        ...
        string fileName = path[(index + 1)..].Replace("Service.cs", "").Replace(".cs", "");

        if (!file.IsRemotePath)
        {
            _log.LogTrace(options.OnlyIdTypes ? $"Get ID types for {fileName} -- START" : $"Get PDS table for {fileName} -- START");
            ...
            try
            {
                ...
                else
                {
                    ...
                    GetTableNames(code, options);

                    Log.Logger.Information("PDS tables found: {a}", _serviceInfo.Tables.Count);
                }

                _log.LogTrace(options.OnlyIdTypes ? $"Get ID types for {_serviceInfo.Name} -- END" : $"Get PDS table for {_serviceInfo.Name} -- END");
            ...
            {
                _log.LogTrace(options.OnlyIdTypes ? $"Get ID types for {fileName} -- END" : $"Get PDS table for {fileName} -- END");
        ...
        else
        {
            ...
            if (file.Path.Contains("Nic.Apis."))
            {
                var serviceInfo = FileService.GetServiceInfo(file.Path);

                code = FileService.GetCodeFromGit(new()
                {
                    Path = file.Path,
            ...
            {
                _serviceInfo.Name = fileName;

                code = FileService.GetCodeFromTfs(new()
                {
                    Path = file.Path,
                    IsRemotePath = file.IsRemotePath
                });
    ...
    private void GetTableNames(string code, ListTablesServiceOptions options)
    {
        ...
        foreach (var clazz in classes)
        {
            foreach (var func in clazz.Functions)
            {
                ...
                foreach (var l in lines)
                {
                    ...
                    {
                        PdsTableService _listTables = new(_log);
                        _serviceInfo.Tables.AddRange(_listTables.ListTablesService(new() { Path = "MicroListPersonServiceAccessRecords" }, options).Tables);
                    }
        ...
        _log.LogInfo($"Get tables from {_serviceInfo.Name}. Number of PDS Tables found {_serviceInfo.Tables.Count}");
        ...
        {
            _log.LogWarning($"Get tables from {_serviceInfo.Name}. No PDS Tables found for \"{_serviceInfo.Name}\"");
        }
        else
        {
            ...
            if (words[i].Equals("FROM") || words[i].Equals("JOIN"))
            {
                ...
                if (!words[i + 1].Equals("\"") && !words[i + 1].Contains('.'))
                {
                    if (!wordsNotCleaned[i + 1].ToUpper().StartsWith("DBO."))
                    {
                        ...
                        _log.LogError($"Table {words[i + 1]} doesn't start with dbo.");
                    }
                    ...
                    else
                    {
                        ...
                        _log.LogError($"Table {words[i + 1]} doesn't have \"with(nolock)\".");
                    }
    ...
    private void GetIDTypes(ListTablesServiceOptions options, string fileName, string modelPath, string code)
    {
        ...
        else
        {
            ...
            using var dialog = new SelectProp(microInputs);
            var result = dialog.ShowDialog();

            propNameLog = dialog.selectedProp?.Name; // for logging no need for ToLower().
        ...
        _log.LogInfo("Looping the MicroService -- START");
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            {
                _log.LogTrace("");
                _log.LogTrace($"8000: I found validation contains {propNameLog} input");
                // remove sapces
                ...
                if (line.Contains('>'))
                {
                    _log.LogTrace("8001: I found validation contains '>'");

                    if (!line.Contains($"{propName}>"))
                    {
                        _log.LogTrace($"8002: but it's not for {propNameLog}");
                        continue;
                    }
                    else
                    {
                        _log.LogTrace($"8003: yeah, it is '{propNameLog} >'");

                        // Get the remaining lines
                        var tempCode = lines.Skip(i).ToArray();

                        // Get the first body
                        _log.LogTrace($"8004: get the body for this validation (if-statement)");
                        var ifBody = ModelExtractionService.GetBody(string.Join("\n", tempCode));

                        if (ifBody.Contains("guardsingleid"))
                        {
                            _log.LogTrace($"8005: the body contains 'GuardSingleID()' validation");
                            ...
                            {
                                _log.LogTrace($"8007: sad the validation is for {tempPropName} :(... the GuardSingleID param is not {propNameLog}");
                                continue;
                            }
                            ...
                            _log.LogTrace($"8007: yeah.. the validation is for {propNameLog}");
                            _log.LogTrace($"8008: validationLine: GuardSingleID({formattedLine};");
                        ...
                        {
                            _log.LogWarning("9001: I'm HERE, not implemented yet :(");
                        }
                        else
                        {
                            _log.LogTrace($"8009: the body doesn't contain 'GuardSingleID()' validation");
                        }
                    }
                }
                else if (line.Contains($"{propName}.startswith("))
                {
                    _log.LogTrace($"8016: yeaaaah, I found validation for '{propNameLog}.StartsWith(\"*\")'");
        ...
        {
            _log.LogTrace($"8010: no types and validation found for the {propNameLog}");

            microWS = regex.Replace(microWS, " ").Replace(" ", "").Replace(".tostring()", "");

            if (microWS.Contains("guardsingleid"))
            {
                _log.LogTrace($"8011: the WS contains 'GuardSingleID()' validation without checking for the input");
                ...
                if (tempPropName != propName)
                {
                    _log.LogTrace($"8012: sad the validation is for {tempPropName} :(... the GuardSingleID param is not {propNameLog}");
                }
                else
                {
                    // to be sure that we are in right validation -- END
                    ...
                    _log.LogTrace($"8013: yeah.. the validation is for {propNameLog}");
                    _log.LogTrace($"8014: validationLine: GuardSingleID({formattedLine};");
            ...
            {
                _log.LogTrace($"8015: the WS doesn't contain 'GuardSingleID()' validation");
            }
        }

        _log.LogTrace("Looping the MicroService -- END");

        if (idTypesFounded.Count > 0)
        {
            _log.LogSuccess($"ID type(s) found for {propNameLog}: \n- {string.Join("\n- ", idTypesFounded)}");

            _serviceInfo.Tables = idTypesFounded.ToList();
        }
        else
        {
            _log.LogError($"No ID types found for {propNameLog}");
        }
    }
