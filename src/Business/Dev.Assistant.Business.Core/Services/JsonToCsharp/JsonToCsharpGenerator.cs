...
public class JsonToCsharpGenerator
{
    ...
    private readonly Dictionary<string, string> _classResults = new();

    public JsonToCsharpGenerator() => _options = new JsonToCsharpOptions();
    ...
    private string CreateEntryAsDictionary(string name, ICharReader reader, Lexer lexer)
    {
        ...
        void SkipBlock()
        {
            while (lexer.Token.tokenType is not TokenType.R_Brace
                   and not TokenType.Comma)
            {
                lexer.Advance();
    ...
    private string CreateImmutableClass(string name, IReadOnlyList<(string name, string type)> items)
    {
        ...
        foreach (var item in items)
        {
            ...
            builder += $"public {item.type} {item.name.SnakeToUpperCamel()} {{ get; set; }}";
        }
    ...
    private (string, string) GetItem(ICharReader reader, Lexer lexer)
    {
        ...
        switch (lexer.Token.tokenType)
        {
            ...
            case TokenType.L_Bracket:
                ...
                type = $"{_options.ListType}<{genericType}>";
                break;
    ...
    private string GetArrayGenericType(string key, ICharReader reader, Lexer lexer)
    {
        ...
        if (token.tokenType == TokenType.L_Brace)
        {
            ...
            {
                arrayEntryName = arrayEntryName[..^1];
            }
            else
            {
    ...
    private static Exception CreateException(ICharReader fileReader, string message)
        => new($"[Error] {fileReader.CurrentLine}:{fileReader.CurrentLineOffset} {message}");
}

internal static class Error
{
    internal static Exception UnexpectedToken(ICharReader fileReader, string expected, string actual)
        => new($"[Error] {fileReader.CurrentLine}:{fileReader.CurrentLineOffset}" +
